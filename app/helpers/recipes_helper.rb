	...
	def make_admin_html_code(object_list = [])
	  ...
	  object_list.each_slice(2) do |rec_block|
	  	...
	  	rec_block.each do |rec|
	  		rec_id = rec.id.to_s
				html_code += '<div class="span6"><div class="well"><button type="button" class="btn btn-medium btn-success disabled pull-right" disabled="disabled">' + rec.meal_class + '</button><h4 class="text-center">' + link_to(rec.name.capitalize, :controller => :recipes, action:  :show, id: rec.id) + '</h4><hr><div class="image-div">'
	  ...
	  object_list.each_slice(2) do |rec_block|
	  	...
	  	rec_block.each do |rec|
	  		...
	  		html_star += '</span>'
				...
				html_code+='<button type="button" class="btn btn-medium btn-success disabled pull-right" disabled="disabled">' + rec.meal_class + '</button><h4 class="text-center">' + link_to(rec.name.capitalize, :controller => :recipes, action:  :show, id: rec.id) + '</h4><hr><div class="row"><div class="span3 image-div">'
